...
function Show-FatalError {
    ...
    try {
        ...
        [System.Windows.MessageBox]::Show(
            "HypeTek CPU Power Control konnte nicht gestartet werden.`n`n$detail`n`nLogdatei:`n$LogFile",
            'HypeTek CPU Power Control – Startfehler',
            [System.Windows.MessageBoxButton]::OK,
            [System.Windows.MessageBoxImage]::Error
        ) | Out-Null
    } catch {
        Write-Host "HypeTek CPU Power Control – Startfehler" -ForegroundColor Red
...
try {
    ...
    if ($env:OS -ne 'Windows_NT') {
        throw 'HypeTek CPU Power Control benötigt Windows 10 oder Windows 11.'
    }
